...
async function evaluateAnswer(question: string, expected: string, actual: string) {
  const data = await callOllama([{ role: "system", content: `Score 0-10. Return JSON: {"score": N, "reasoning": "..."}` }, { role: "user", content: `Q: ${question}\nExpected: ${expected}\nActual: ${actual}` }]);
  if (!data) return { score: 0, reasoning: "Evaluation failed" };
...
async function getAIAnswer(question: string, systemPrompt: string) {
  const data = await callOllama([{ role: "system", content: systemPrompt }, { role: "user", content: question }]);
  ...
  try {
    ...
    if (action === "run") {
      ...
      for (const q of questions) {
        const answer = await getAIAnswer(q.question, systemPrompt);
        const evaluation = await evaluateAnswer(q.question, q.expected_answer || "", answer);
      ...
      await supabase.from("benchmark_runs").insert({ user_id: userId, total_score: normalizedScore, max_score: 100, category_scores: catScoresNormalized, model_config: { model: OLLAMA_MODEL, prompt_version: systemPromptVersion || 1 }, system_prompt_version: systemPromptVersion || 1 });
