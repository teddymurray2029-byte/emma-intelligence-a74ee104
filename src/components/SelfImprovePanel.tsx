...
export function SelfImprovePanel() {
  ...
  const [selectedRepo, setSelectedRepo] = useState<string>(() => localStorage.getItem("emma-improve-repo") || "");
  const [pushing, setPushing] = useState(false);

  useEffect(() => {
    listGitHubRepos().then((r) => setRepos(r.data || [])).catch(() => {});
  }, []);
  ...
  const handlePushGitHub = async () => {
    ...
    setPushing(true);
    localStorage.setItem("emma-improve-repo", selectedRepo);
    try {
      const w = analysis.proposal;
      const t = analysis.tournament;
      const version = analysis.nextPromptVersion;
      const md = `# Emma Self-Improvement v${version}

**Winner:** ${w.candidateType} / ${w.diffType}
**Tournament score:** ${w.total.toFixed(2)} / 10
**Predicted delta:** ${w.predictedDelta > 0 ? "+" : ""}${w.predictedDelta} pts
**Current benchmark:** ${analysis.currentScore}/100
...
## Judge Scores
| Coherence | Novelty | Safety | Impact | Feasibility |
|-----------|---------|--------|--------|-------------|
| ${w.scores.coherence} | ${w.scores.novelty} | ${w.scores.safety} | ${w.scores.impact} | ${w.scores.feasibility} |

## Weak Categories
${analysis.weakCategories.map((c) => `- ${c}`).join("\n") || "_none_"}

## Tournament Bracket (${t.candidateCount} candidates, ${t.diversityBuckets.length} buckets)
${t.rankings.map((r) => `${r.rank}. **${r.candidateType}** — ${r.proposal} _(${r.total.toFixed(2)}, predicted ${r.predictedDelta > 0 ? "+" : ""}${r.predictedDelta})_`).join("\n")}

---
_Auto-generated by Emma self-improvement tournament._
`;
      const res = await pushGitHubFiles(
        selectedRepo,
        [{ path: `improvements/v${version}.md`, content: md }],
        `chore(emma): self-improve v${version} — ${w.candidateType} (+${w.predictedDelta} predicted)`,
      );
      toast.success(`Pushed to ${selectedRepo} · ${res.sha?.slice(0, 7)}`);
    } catch (err: any) {
      toast.error(err.message);
    }
    setPushing(false);
    ...
    <div className="space-y-4">
      ...
      {analysis && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
          ...
          <div className="emma-surface-elevated rounded-xl p-3 space-y-2">
            <p className="text-[10px] font-mono text-muted-foreground flex items-center gap-1">
              <Github className="h-2.5 w-2.5" />PUSH IMPROVEMENT TO GITHUB
            </p>
            <Select value={selectedRepo} onValueChange={setSelectedRepo}>
              <SelectTrigger className="h-8 text-xs">
                <SelectValue placeholder={repos.length ? "Select repository..." : "Loading repos..."} />
              </SelectTrigger>
              <SelectContent>
                {repos.map((r) => (
                  <SelectItem key={r.full_name} value={r.full_name} className="text-xs">{r.full_name}</SelectItem>
                ))}
              </SelectContent>
            </Select>
            <Button onClick={handlePushGitHub} disabled={pushing || !selectedRepo} variant="outline" className="w-full h-9 text-xs" size="sm">
              {pushing ? (
                <><Loader2 className="h-3 w-3 mr-2 animate-spin" />Pushing to {selectedRepo}...</>
              ) : (
                <><Github className="h-3 w-3 mr-2" />Commit v{analysis.nextPromptVersion} to GitHub</>
              )}
            </Button>
            <p className="text-[9px] text-muted-foreground">Writes <span className="font-mono text-foreground">improvements/v{analysis.nextPromptVersion}.md</span> on the default branch.</p>
          </div>
        </motion.div>
      )}
    </div>
