...
async function callAI(apiKey: string, messages: any[], model = "google/gemini-2.5-flash"): Promise<string> {
  const resp = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", { method: "POST", headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" }, body: JSON.stringify({ model, messages }) });
...
async function causalInference(apiKey: string, phenomenon: string) {
  const raw = await callAI(apiKey, [{ role: "system", content: `Causal inference engine. Return JSON with variables, causalGraph, rootCauses, interventions, counterfactuals, confidence.` }, { role: "user", content: `Analyze: ${phenomenon}` }], "google/gemini-2.5-pro");
...
async function architecturalAnalysis(apiKey: string, supabase: any, userId: string) {
  ...
  const raw = await callAI(apiKey, [{ role: "system", content: `AI architecture optimizer. Return JSON with bottlenecks, proposedUpgrades, architectureScore, selfModificationPlan.` }, { role: "user", content: `State: ${JSON.stringify({ memoryEpisodes: memCount.count || 0, activeGoals: goalCount.count || 0, recentBenchmarks: benchData.data || [], recentImprovements: improvData.data || [] })}` }], "google/gemini-2.5-pro");
...
async function groundedReasoning(apiKey: string, scenario: string) {
  const raw = await callAI(apiKey, [{ role: "system", content: `Grounded reasoning engine. Return JSON with physicalModel, agentModel, temporalChain, groundingScore.` }, { role: "user", content: `Reason about: ${scenario}` }], "google/gemini-2.5-pro");
